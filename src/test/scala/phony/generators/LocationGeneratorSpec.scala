...
class LocationGeneratorSpec extends FunSuite with MockFactory {
  ...
  val generator = new LocationGenerator[IO]

  test("It should generate a latitude") {
    (random.nextDouble _).expects().returning(IO(0.6211648765055653))
    generator.latitude
  ...
  test("It should generate a longitude") {
    (random.nextDouble _).expects().returning(IO(0.3682367445255824))
    generator.longitude
  ...
  test("It should select one country name from the available countries") {
    (random.randomItem(_: Seq[Country])).expects(dataProvider.location.countries).returning(IO(Country("Iran", "IR")))
  ...
  test("It should select one country from the available countries") {
    (random.randomItem(_: Seq[Country])).expects(dataProvider.location.countries).returning(IO(Country("Germany", "DE")))
    generator.country
