...
class LocationGenerator[F[_]: Monad](implicit val utility: RandomUtility[F], locale: Locale[F]) {
  ...
  def countryName: F[String] =
    locale.location.map(_.countries).flatMap(utility.randomItem).map(_.name)

  def countryCode: F[String] =
    locale.location.map(_.countries).flatMap(utility.randomItem).map(_.code)

  def country: F[Country] =
    locale.location.map(_.countries).flatMap(utility.randomItem)
