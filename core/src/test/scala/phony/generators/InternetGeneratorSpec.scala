...
class InternetGeneratorSpec extends AnyFunSuite {
  ...
  test("It should generate an Email") {
    (generator.email.success.value should fullyMatch).regex(
  ...
  test("It should generate a valid UUID") {
    (generator.uuid.map(_.toString).success.value should fullyMatch)
